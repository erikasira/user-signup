...
@app.route("/")
def sign_up():
    return render_template('form.html')                     #this is the first page of the program, we are rendering the form from the templates that was written in html
...
#THIS IS FOR EMAIL
    ...
    if not username_error and not password_error and not verify_error and not email_error:                      #if there is no error in the username send them to the correct page                                                                    
        ...
        return render_template('form.html', username=username, username_error=username_error, password_error=password_error, verify_error=verify_error, email_error=email_error, email =email)   #otherwise, give the user the form with the username in tact
